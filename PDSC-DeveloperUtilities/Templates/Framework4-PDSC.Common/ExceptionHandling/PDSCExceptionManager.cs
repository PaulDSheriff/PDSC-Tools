  ...
  public class PDSCExceptionManager : CommonBase {
    ...
    public virtual void SetDatabaseInformation() {
      if (DbContextObject != null) {
        ExceptionObject.ConnectionString = HideLoginInfoForConnectionString(DbContextObject.Database.Connection.ConnectionString);
        ExceptionObject.DatabaseName = DbContextObject.Database.Connection.Database;
        ExceptionObject.SqlServer = ExceptionObject.SqlServer ?? DbContextObject.Database.Connection.DataSource;
        ExceptionObject.WorkstationId = ((SqlConnection)DbContextObject.Database.Connection).WorkstationId;
      }
    ...
    public virtual string HideLoginInfoForConnectionString(string connectString) {
      if (!string.IsNullOrEmpty(connectString)) {
        SqlConnectionStringBuilder cnb = new SqlConnectionStringBuilder(connectString);
